...
### 어떤 조건이 성과를 만들었나 (국내주식 9종목, 2016~2026, 27개 조합)

| 구간 거래량 기준 (`vol_ratio_min`) | 거래 수 | 평균 R | 손익비 | 플러스 종목 |
|---|---|---|---|---|
| 0.7 (평소보다 적어도 통과) | 47~79 | −0.03 ~ +0.03 | 0.96~1.15 | 3~5/9 |
| **1.0 (평소 이상, 기본값)** | 20~26 | +0.19 ~ +0.28 | 1.85~2.37 | 2~4/9 |
| 1.3 (평소의 1.3배 이상) | 4~6 | +0.41 ~ +0.64 | 9.5 이상 | 2/9 |

영상의 핵심 주장인 **"평평한 가격 + 평소보다 많은 거래량 = 매집"** 필터를 빼면 기대값이 0 근처로 떨어지고, 세게 걸수록 거래는 줄지만 건당 기대값은 올라갔습니다.
익절 배수(1.5R·2R·3R)는 결과를 거의 바꾸지 않았고, 자금 흐름 지표는 ADL 이 OBV 보다 플러스 종목 수가 조금 많았습니다(4/9 vs 2/9). 다만 어느 조합도 거래 수가 적어 확신할 단계는 아닙니다.

## 확인 못 한 것 · 한계

- 업비트 수수료 0.05%는 API 문서 예시값과 ccxt 내장값에 근거합니다. 공식 수수료 안내 페이지는 서버에서 열리지 않아(403) 직접 확인하지 못했습니다. 실행 시 `fetch_trading_fee` 로 확인하세요.
- 업비트에 손절·익절 주문이 없다는 것은 주문 종류(`ord_type`: limit · price · market · best)에 스톱 계열이 없다는 데서 추론한 것입니다. 공식 문서에 "지원하지 않는다"는 문장은 없습니다.
- 실거래 주문 코드(`UpbitBroker`)는 키가 없어 실제 체결까지 시험하지 못했습니다. 첫 실거래는 최소 금액(5,000원 근처)으로 한 번 해 보고 상태 파일과 체결 내역을 대조하세요.
- 야후 파이낸스 주가는 20분 지연·개인 용도 데이터입니다. 백테스트용으로만 쓰세요.
- 백테스트는 지금 존재하는 종목만 대상으로 했습니다(상장폐지 종목이 빠진 생존 편향). 파라미터를 여러 번 바꿔 본 만큼 과최적화 위험도 있습니다.
- 돌파 거래량 기준의 학계·차트 교과서 통상값은 2.0~2.5배인데, 이 데이터에서는 그 값이 성과를 깎아 기본을 1.0배로 두었습니다. 시장이 바뀌면 다시 확인해야 합니다.
